...
This results into regionsl p-values that measure sthe strnegth of association
...
def analysis(X, y, k, session, do_classif=False, do_rsa=False, do_lm=False,
    ...
    k: int,
       indicates the region (set of features) on which the analysis is run

    session: array of indexes
             session index (used for clean cross validation)

    do_classif: bool,
                whether to run classification or not

    do_rsa: bool,
            whether to run rsa or not

    do_lm: bool
           whether to run linear model or not
